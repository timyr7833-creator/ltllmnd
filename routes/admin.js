    ...
    try {
        ...
        if (result.rows.length === 0) {
            return res.render('login', {
                error: 'Неверные учётные данные', page: 'login'
            });
        }
        ...
        console.error(err);
        res.render('login', {
            error: 'Ошибка сервера', page: 'login'
        });
    }
});
...
router.post('/admin/products/add', requireAdmin,
    upload.array('images', 10),
    async (req, res) => {
        try {
            const {
                name, description, price, old_price,
                category, sizes, in_stock, featured
            } = req.body;
            ...
            const result = await pool.query(`
                ...
                (name, slug, description, price, old_price, category, sizes, in_stock, featured)
...
router.post('/admin/products/edit/:id', requireAdmin,
    upload.array('images', 10),
    async (req, res) => {
        try {
            const {
...
router.get('/admin/orders', requireAdmin, async (req, res) => {
    try {
        ...
        // Подгрузим позиции для каждого заказа
...
router.post('/admin/orders/:id/status', requireAdmin, async (req, res) => {
    const allowed = ['new', 'confirmed', 'paid', 'shipped', 'delivered', 'cancelled'];
